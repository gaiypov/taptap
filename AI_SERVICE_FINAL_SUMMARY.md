# AI Service with API Keys - Final Update Summary

## ✅ Полная реализация AI сервиса с ключами

### 📁 Созданные файлы:
```
services/ai/
├── config.ts          # Конфигурация и API ключи
├── utils.ts           # Утилиты для работы с AI
├── index.ts           # Основной AI сервис
├── claude.ts          # Claude API интеграция
├── openai.ts          # OpenAI API интеграция
└── google.ts          # Google Vision API интеграция

services/ai.ts         # Главный экспорт (обновлен)
```

## 🔧 Обновленный services/ai.ts

### Основные функции:
```typescript
// Главная функция анализа
export async function analyzeCarVideo(
  videoUri: string,
  onProgress?: (stage: string, progress: number) => void
): Promise<Partial<Car>>

// Быстрая идентификация
export async function quickIdentifyCar(imageUri: string): Promise<CarInfo>

// Проверка качества видео
export async function validateVideoQuality(videoUri: string): Promise<ValidationResult>

// Сравнение автомобилей
export async function compareCars(car1: any, car2: any): Promise<ComparisonResult>
```

### Логика работы:
1. **Проверка API ключей** - `checkAPIKeys()`
2. **Выбор AI провайдера** - `selectAvailableAI()`
3. **Mock режим** - если нет ключей или включен USE_MOCK
4. **Извлечение кадров** - максимум 3 изображения
5. **AI анализ** - через выбранный провайдер
6. **Логирование стоимости** - `logAPICost()`
7. **Fallback на mock** - при ошибках в development

## 🤖 Claude API (services/ai/claude.ts)

### Функции:
- **`analyzeWithClaude()`** - Полный анализ автомобиля
- **`quickIdentifyWithClaude()`** - Быстрая идентификация

### Особенности:
- Использует Claude-3-Sonnet для полного анализа
- Использует Claude-3-Haiku для быстрой идентификации
- Поддержка множественных изображений
- Детальный промпт для анализа автомобиля
- Автоматический парсинг JSON ответов

## 🧠 OpenAI API (services/ai/openai.ts)

### Функции:
- **`analyzeWithOpenAI()`** - Полный анализ
- **`quickIdentifyWithOpenAI()`** - Быстрая идентификация
- **`detectDamagesWithOpenAI()`** - Обнаружение повреждений

### Типы анализа:
- **full_analysis** - Полный анализ автомобиля
- **quick_identify** - Быстрая идентификация
- **damage_detection** - Обнаружение повреждений

### Особенности:
- Использует GPT-4 Vision для анализа
- Поддержка высокого качества изображений
- Специализированные промпты для разных задач
- Автоматический fallback при ошибках

## 👁️ Google Vision API (services/ai/google.ts)

### Функции:
- **`analyzeWithGoogleVision()`** - Полный анализ
- **`extractMileageWithGoogle()`** - Извлечение пробега
- **`detectDamagesWithGoogle()`** - Обнаружение повреждений

### Типы анализа:
- **full** - Полный анализ с всеми функциями
- **ocr** - Извлечение текста (пробег, год)
- **object_detection** - Обнаружение объектов
- **text_detection** - Обнаружение текста

### Функции Google Vision:
- **LABEL_DETECTION** - Определение марки, цвета
- **TEXT_DETECTION** - Извлечение пробега, года
- **OBJECT_LOCALIZATION** - Обнаружение повреждений
- **WEB_DETECTION** - Дополнительная информация

## 🔑 Конфигурация ключей

### Переменные окружения:
```bash
# .env файл
EXPO_PUBLIC_OPENAI_API_KEY=sk-your-openai-key
EXPO_PUBLIC_CLAUDE_API_KEY=sk-ant-your-claude-key
EXPO_PUBLIC_GOOGLE_API_KEY=AIza-your-google-key
EXPO_PUBLIC_ROBOFLOW_API_KEY=your-roboflow-key

# Конфигурация
EXPO_PUBLIC_AI_MODE=development
EXPO_PUBLIC_USE_MOCK=true
```

### app.json:
```json
{
  "extra": {
    "EXPO_PUBLIC_AI_MODE": "development",
    "EXPO_PUBLIC_USE_MOCK": "true"
  }
}
```

## 💰 Оптимизация расходов

### Настройки экономии:
```typescript
const AI_CONFIG = {
  MAX_IMAGES_PER_ANALYSIS: 3,    // Только 3 лучших кадра
  IMAGE_QUALITY: 0.7,            // Сжатие до 70%
  ENABLE_CACHING: true,          // Кэширование результатов
  PRIMARY_AI: 'claude',          // Приоритет Claude (дешевле)
};
```

### Стоимость по провайдерам:
- **Claude** - $0.023 за изображение
- **OpenAI** - $0.03 за изображение  
- **Google** - $0.0015 за изображение
- **Mock** - $0 (бесплатно)

### Логирование стоимости:
```typescript
logAPICost('claude', 3); // $0.069 (claude, 3 images)
```

## 🚀 Использование

### Базовое использование:
```typescript
import { analyzeCarVideo, quickIdentifyCar, AI_CONFIG, aiUtils } from '@/services/ai';

// Проверка конфигурации
aiUtils.logAIConfiguration();

// Анализ автомобиля
const result = await analyzeCarVideo(videoUri, (stage, progress) => {
  console.log(`${stage}: ${progress}%`);
});

// Быстрая идентификация
const carInfo = await quickIdentifyCar(imageUri);
```

### Проверка статуса:
```typescript
const status = aiUtils.getAIStatus();
console.log('Selected provider:', status.selectedProvider);
console.log('Has API keys:', status.hasKeys);
console.log('Available providers:', status.availableProviders);
```

## 🔄 Режимы работы

### Development режим:
```json
{
  "EXPO_PUBLIC_AI_MODE": "development",
  "EXPO_PUBLIC_USE_MOCK": "true"
}
```
- Подробное логирование
- Mock режим по умолчанию
- Fallback на mock при ошибках
- Отображение стоимости запросов

### Production режим:
```json
{
  "EXPO_PUBLIC_AI_MODE": "production",
  "EXPO_PUBLIC_USE_MOCK": "false"
}
```
- Минимальное логирование
- Реальные AI провайдеры
- Строгие ошибки без fallback
- Оптимизация производительности

## 🛠️ Mock режим

### Автоматический mock:
- Включается при отсутствии API ключей
- Включается при `USE_MOCK: true`
- Используется как fallback при ошибках

### Mock данные:
```typescript
function getMockCarData(): Partial<Car> {
  return {
    brand: 'Toyota',
    model: 'Camry',
    year: 2020,
    mileage: 45120,
    location: 'Бишкек',
    aiAnalysis: {
      condition: 'good',
      conditionScore: 85,
      damages: [
        {
          type: 'scratch',
          severity: 'minor',
          location: 'правая дверь',
          confidence: 0.87,
        },
      ],
      estimatedPrice: { min: 2400000, max: 2600000 },
      features: ['Кожаный салон', 'Камера'],
    },
  };
}
```

## 🔒 Безопасность

### ⚠️ ВАЖНО: API ключи в клиенте
**Этот подход НЕ рекомендуется для продакшена!**

### Проблемы:
- API ключи могут быть извлечены из приложения
- Нет контроля над использованием ключей
- Нарушение условий использования AI провайдеров
- Потенциальные утечки в production

### ✅ Рекомендуемое решение:
Используйте **backend API** (уже реализован) для хранения ключей на сервере.

## 📊 Мониторинг

### Логирование:
```typescript
// В development режиме автоматически логируется:
console.log('🔑 Available APIs:', availableKeys);
console.log(`🤖 Using AI: ${selectedAI}`);
console.log(`💰 AI Cost: $${cost.toFixed(4)} (${provider}, ${imageCount} images)`);
```

### Метрики:
- Количество запросов к каждому провайдеру
- Общая стоимость AI запросов
- Эффективность кэширования
- Время отклика провайдеров
- Успешность запросов

## 🛠️ Troubleshooting

### Частые проблемы:

1. **"No AI API keys found"**
   ```typescript
   const keys = checkAPIKeys();
   console.log('Available keys:', keys);
   ```

2. **"Invalid API key format"**
   - OpenAI: должен начинаться с `sk-`
   - Claude: должен начинаться с `sk-ant-`
   - Google: должен начинаться с `AIza`

3. **"High API costs"**
   ```typescript
   // Уменьшите количество изображений
   MAX_IMAGES_PER_ANALYSIS: 2
   
   // Включите кэширование
   ENABLE_CACHING: true
   ```

## 🎯 Рекомендации

### Для разработки:
- Используйте `USE_MOCK: true`
- Включите подробное логирование
- Тестируйте с разными провайдерами
- Мониторьте стоимость запросов

### Для продакшена:
- **НЕ используйте API ключи в клиенте**
- Используйте backend API (уже реализован)
- Настройте мониторинг расходов
- Ограничьте количество запросов
- Используйте кэширование

## ✅ Готово к использованию

- ✅ Полная интеграция с Claude API
- ✅ Полная интеграция с OpenAI API
- ✅ Полная интеграция с Google Vision API
- ✅ Автоматический выбор провайдера
- ✅ Оптимизация расходов
- ✅ Кэширование результатов
- ✅ Mock режим для разработки
- ✅ Fallback механизмы
- ✅ Подробное логирование
- ✅ Типизация обновлена
- ✅ Документация создана

Теперь AI сервис полностью готов для работы с реальными API ключами! 🚀

**Примечание**: Для продакшена рекомендуется использовать backend API вместо клиентских ключей.
